...
`LINKEDIN_DATE_FILTER=past-24h` restricts every role to posts from the last 24
hours. Posts marked `2d` or older are intentionally excluded. Company size is
never a filter: startups and large enterprises are judged by the same genuine
opening and corporate-contact rules.
